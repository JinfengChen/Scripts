...
our ($query,$target,$blastm8,$qrygenegff,$qryrepeatgff,$refgenegff,$refrepeatgff,$identity,$lencut,$help);
GetOptions(
         "query:s" => \$query, 
         "target:s"=> \$target, 
         "blastm8:s"=> \$blastm8,
         "qrygenegff:s"=> \$qrygenegff,
         "qryrepeatgff:s"=> \$qryrepeatgff,
         "refgenegff:s"=> \$refgenegff,
         "refrepeatgff:s"=> \$refrepeatgff,
         "identity:i"=> \$identity,
         "lencut:i" => \$lencut,
         "help:s"   => \$help
);
...
my $scaf2gene;
if (-e $refgenegff) {
     $scaf2gene=parsegene($refgenegff);
}
my $scaf2te;
if (-e $refrepeatgff) {
     $scaf2te  =parsete($refrepeatgff);
}

our $bac2gene;
if (-e $qrygenegff) {
     $bac2gene=parsegene($qrygenegff);
}
our $bac2te;
if (-e $qryrepeatgff) {
     $bac2te  =parsete($qryrepeatgff);
}
...
open OUT, ">../output/align/$_.fas" or die "$!";
       ...
       foreach (sort keys %{$verify2scaf{$verify}}){
           ...
           if ($hstart-10000 > 0){ #### add 10k to the start and end of hit region
               $hstart=$hstart-10000;  
           }else{
               $hstart=0;
           }
           if ($hend+10000 < $scaflen{$scaffold}){
               $hend=$hend+10000;
           }else{
...
foreach (@$refgap){
     ...
     my $gap=$svg->rectangle(
         ...
         if (-e $qrygenegff){
             print "Draw gene annotation\n";
             my $refgene=$bac2gene->{$verify};
             foreach (@$refgene){
                    my @unit=split("\t",$_);
                    my ($genestart,$geneend,$genelen);
                    if(1){
                            $genestart=$unit[0]/$scale+$startw;
                            $geneend  =$unit[1]/$scale+$startw;
                            #$genelen  =abs($geneend-$genestart)+1;
                            $genelen  =abs($unit[1]-$unit[0]+1)/$scale;
                    }
                    my $gene=$svg->rectangle(
                                     x=>$genestart,y=>$starth-15,
                                     width=>$genelen,height=>5,
                                     style=>{
                                         fill=>'red'
                                     }
         ...
         if (-e $qryrepeatgff){
             print "Draw TE annotation\n";
             my $refte=$bac2te->{$verify};
             foreach (@$refte){
                    my @unit=split("\t",$_);
                    my ($testart,$teend,$telen);
                    if (1){
                            $testart=$unit[0]/$scale+$startw;
                            $teend  =$unit[1]/$scale+$startw;
                            #$telen  =abs($teend-$testart)+1;
                            $telen  =abs($unit[1]-$unit[0]+1)/$scale;
                    }
                    my $te=$svg->rectangle(
                                     x=>$testart,y=>$starth-25,
                                     width=>$telen,height=>5,
                                     style=>{
                                         fill=>'blue'
                                     }
...
close OUT;
       ...
       #/home/jfchen/159/FFproject/tools/draw/svg2xxx_release/svg2xxx
       #system "/home/jfchen/159/FFproject/tools/draw/svg2xxx_release/svg2xxx $file -t pdf -m 200";
}
