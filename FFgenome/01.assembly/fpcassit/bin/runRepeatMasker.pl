...
open OUT, ">$fdshell" or die "$!";
     print OUT "$fastadeal -cutf 90 $infile -outdir $outdir\n";
close OUT;
`$qsub $fdshell`;
...
foreach (@subfiles){
    print "$_\n";
    print OUT "$repeatmasker -lib $lib -xsmall -nolow -no_is -norna -engine wublast $_ > $_.log 2> $_.log2\n";
}
close OUT;

## run shell by qsub-sge.pl
`$qsub --maxjob 30 --resource vf=1.9G $repeatshell`;
