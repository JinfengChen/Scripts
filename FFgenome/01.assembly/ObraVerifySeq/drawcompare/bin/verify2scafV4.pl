...
#open OUT, ">../output/align/$_.fas" or die "$!";
       ...
       foreach (sort keys %{$verify2scaf{$verify}}){
           ...
           if ($hstart-20000 > 0){ #### add 10k to the start and end of hit region
               $hstart=$hstart-20000;  
           }else{
               $hstart=0;
           }
           if ($hend+20000 < $scaflen{$scaffold}){
               $hend=$hend+20000;
           }else{
           ...
           if ($addlen < 1000){
                  ...
                  open OUT, ">>HitPosition4Verify.txt" or die "$!";
                      print OUT "$verify\t$scaffold\t$hstart\t$hend\t$addlen\n";
                  close OUT;
                  #####
           } 
...
close OUT;
       system "/share/raid12/chenjinfeng/tools/draw/svg2xxx_release/svg2xxx $file -t pdf -m 200";
}
