...
sub get_the_map()
{
   ...
   foreach my $name (keys(%contig_compose))
   {
      ...
      my $width=800;#add by chen
      my $bp_per_pix=$length/($width-100); #my $bp_per_pix=20
      
      my $svg=SVG->new(width=>$width,height=>100);#my $svg=SVG->new(width=>($length*10)/(9*$bp_per_pix),height=>500)
      my $path=$svg->group(
      ...
      foreach my $key_1 (sort {$a<=>$b} keys %{$contig_compose{$name}})
      {
         ...
         $rect->rectangle(
            x=>$x_start+$c_s/$bp_per_pix,y=>60,
            width=>$c_len/$bp_per_pix,height=>3,
         ); 
      }
      &plot_ruler("svg",$svg,"Y",80, "X_start",$x_start,"X_end",$x_start+($end-$start)/$bp_per_pix,"bp_start",$start,"bp_end",$end,"scaletype","Kb","scaletypepos","right","scalestart","force","rulerstyle",2);
