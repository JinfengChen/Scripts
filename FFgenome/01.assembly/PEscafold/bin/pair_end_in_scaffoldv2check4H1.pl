...
sub get_the_map()
{
   ...
   foreach my $name (keys(%contig_compose))
   {
      ...
      my $width=1200;#add by chen
      my $bp_per_pix=2500000/($width-0); #my $bp_per_pix=20
      ...
      if ($ARGV[4] ne ""){   
        ...
        drawcoverage2($svg,$coverage->{$name},280,80,$bp_per_pix);
      }
      ###########################################画出contig和坐标
      my $x_start=280;
      my $c_s=0;my $c_len=0;
