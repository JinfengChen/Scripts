...
use warnings;
#use strict;
GetOptions (\%opt,"gff:s","code:s","project:s","help");
...
my ($genecode,$transcode)=readtable($opt{code});
...
foreach my $g (keys %$gene){
    my $tempgcode=$genecode->{$g} ? $genecode->{$g} : $g;
    $count{$genetype->{$g}->[0]}++;
    ...
    foreach my $t (keys %{$gene->{$g}}){
       ...
       my $strand;
       my $project;
       $temptcode=$transcode->{$t} ? $transcode->{$t} : $t;
