...
foreach my $chr (keys %chrlen){
     ...
     my $size ="$opt{table}/$chr.$opt{suffix}.size";
     print "$table\n";
     next unless (-f $table);
     print "Drawing\n"; 
     ###svg
     ...
     my $ypos=($xpos-100)/2+100;
     my $title="Kb";
     my ($svg,$rate4yaxis2)=drawyaxis2($svg,$end,100,$xpos,$up,$down,$step,$title);
     
     ### draw Os, upward
     my $svg=drawbar($svg,$table,$ypos,$rate,$rate4yaxis,$size,$rate4yaxis2,$chrlen{$chr});  
...
sub drawbar
{
my ($svg,$table,$ypos,$xrate,$yrate,$size,$yrate2,$chrlen)=@_;
####
if (1){
  open IN, "$size" or die "$!";
  while(<IN>){
     ...
     next if ($_=~/^$/);
     my @unit=split("\t",$_);
     my $x1=($unit[1]-1)*1000000/$xrate+100;
     my $x2=$unit[1]*1000000 < $chrlen ? $unit[1]*1000000/$xrate+100 : $chrlen/$xrate+100;
     my $y =$unit[2] > 0 ? $ypos-$unit[2]*$yrate2/1000 : $ypos+$unit[2]*$yrate2/1000;
     my $binline=$svg->line(
     x1=>$x1,y1=>$y,
     x2=>$x2,y2=>$y,
     style=>{stroke=>'blue'}
