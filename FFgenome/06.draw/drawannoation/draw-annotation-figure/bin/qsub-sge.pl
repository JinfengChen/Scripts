...
my ($Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,$Verbose, $Help);
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"interval:i"=>\$Interval,
...
while(<IN>){
	...
	if ($line_mark % $Lines == 0) {
		open OUT,">$Work_dir/work_$Job_mark.sh" || die "failed creat work_$Job_mark.sh";
		push @Shell,"work_$Job_mark.sh";
		$Job_mark++;
	}
