...
my @align=glob("$opt{align}/chr*.aligns");
foreach(@align){
   my $chr;
   if ($_=~/\/(chr\d+)\.aligns/){
      $chr=$1;
...
############# start block and print out head if there was a block interal
       ...
       if ($tempqrystart-$startqry > 10000){
           my $refhead=$ob1."_".$chr."_".$startref."_".$temprefend;
           my $qryhead=$os1."_".$chr."_".$startqry."_".$tempqryend;
...
######################################################################
       ...
       foreach(@$block){
           ...
           if ($qrystart-$startqry >= 4000000){
               my $refhead=$ob1."_".$chr."_".$startref."_".$refend;
               my $qryhead=$os1."_".$chr."_".$startqry."_".$qryend;  
               print "$qryhead\t$refhead\n";
...
###Os01	1023020	1200220	Os01t001010
my ($gff)=@_;
my %hash;
open IN, "$gff" or die "$!";
while(<IN>){
        ...
        my $reftemp=$hash{$unit[0]};
            $reftemp->{$unit[3]}=[$unit[1],$unit[2]];
            $hash{$unit[0]}=$reftemp;
        }else{
            my %temp;
            $temp{$unit[3]}=[$unit[1],$unit[2]];
            $hash{$unit[0]}=\%temp;
        }
}
close IN;
