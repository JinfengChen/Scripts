...
while(<IN>){
    ...
    `perl GetRegionData.pl --qryhead $unit[0] --qrygff3 ../input/OBa.chr.gff.chr --qryfasta ../input/OBa.chr.fa.chr --qryrepeat ../input/OBa.all.manual.TE.chr.gff.chr --refhead $unit[1] --refgff3 ../input/tigr.all.final.gff.chr --reffasta ../input/all.con.chr -refrepeat ../input/all.con.RepeatMasker.out.gff.chr --outdir $outdir1`;
}
close IN;
