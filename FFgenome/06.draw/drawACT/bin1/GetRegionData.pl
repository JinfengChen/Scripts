...
getsubrepeat("$opt{qryrepeat}/$qryinf[1]",,$qryhead,"+");
getsubrepeat("$opt{refrepeat}/$refinf[1]",,$refhead,"+");
`perl GFF2embl.pl -gff $qryhead.gff -embl $qryhead.embl -fasta $qryhead.fasta`;
`perl gffrepeat2embl.pl -repeat $qryhead.repeat -embl $qryhead.embl -title $qryhead`;
`mv $qryhead.embl $qryhead.gene`;
`mv $qryhead.merge $qryhead.embl`;
`perl GFF2embl.pl -gff $refhead.gff -embl $refhead.embl -fasta $refhead.fasta`;
`perl gffrepeat2embl.pl -repeat $refhead.repeat -embl $refhead.embl -title $refhead`;
`mv $refhead.embl $refhead.gene`;
`mv $refhead.merge $refhead.embl`;
`perl runblast2seq.pl`;
`perl run2act.pl`;
`rm *.fasta.n* *.blast *.temp *.fasta *.gene *.repeat`;
`mv $qryhead* $dir/`;
`mv $refhead* $dir/`;
}
#####################
