...
--segment: directory contains segment file that indicate heterchromain region with 1
--project name
=cut
...
our %opt;
GetOptions(\%opt,"chrlen:s","TEfeature:s","GENEfeature:s","methylation:s","segment:s","project:s","help");

if (keys %opt  < 1 or $opt{help}){
    print "perl drawChrFeature.pl --chrlen ../input/OBa.chrlen --TEfeature ../input/OBa_feature_wave --methylation ../input/OBa_methylation_wave --segment ../input/OBa_methylation_wave --project OBa > log 2> log2 &\n";
    exit();
}
...
foreach my $chr (keys %chrlen){
     ...
     ###draw chromosome with heterchromatin distribution
     my $cpos=$height-50; 
     $svg=drawchr($svg,$opt{segment},100,$cpos,$chrlen{$chr},$chr,$rate);
     ###draw methlation
