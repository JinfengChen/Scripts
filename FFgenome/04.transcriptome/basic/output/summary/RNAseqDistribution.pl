...
#`perl $gff2utr -gff $opt{gene} -feature UTR -bed $opt{project}.UTR.bed`;

#`/home/jfchen/FFproject/tools/BEDTools/bin/intersectBed -u -a $opt{bed} -b $opt{project}.mRNA.bed > $opt{project}.mRNA.overlap`;
#`/home/jfchen/FFproject/tools/BEDTools/bin/intersectBed -u -a $opt{bed} -b $opt{project}.UTR.bed > $opt{project}.UTR.overlap`;
...
#my $utrread =`wc -l $opt{project}.UTR.overlap`;
