...
foreach(keys %$refgff){
    ...
    $exoneline.=",";
    my $exonstrandline=join(",",@exonstrand);
    my $temp="$name:$function";
    print OUT "$gene\t$name:$function\t$chr\t$strand\t$tss\t$tes\t$css\t$ces\t$exonnumber\t$exonsline\t$exoneline\t0\t$gene\tNA\tNA\t$exonstrandline\n";
}
close OUT;
