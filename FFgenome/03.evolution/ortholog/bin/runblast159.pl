...
die "Usage: perl $0 -p blastp -f T -i infile -d database > log" if ($help);
die "Usage: perl $0 -p blastp -f T -i infile -d database > log" unless (-f $infile);
...
my @subblast=glob("$outdir/$filename.cut/*.blast");
foreach my $file (@subblast){
    my $head=$1 if ($file=~/(.*)\.blast$/);
    `$blastparser -nohead $file > $head.table`;
}

`cat $outdir/$filename.cut/*.table > $outdir/$parseout`;
