...
$maximum_size ||= 500;
$fam_in_subdir_num ||= 100; ##means 100 families in each subdirectory under top gene_families directory
...
my $blastall = parse_config($config_file,"blastall");
my $solar = parse_config($config_file,"solar");
my $hcluster_sg = parse_config($config_file,"hcluster_sg");
...
if ($Step =~ /2/) {
	...
	`fishInWinter.pl  $hcluster_result_file.stat.single-copy $hcluster_result_file > $hcluster_result_file.single-copy`;
	...
	foreach my $p (@fam) {
		...
		if(-f $tree_nhx_file){
			print OUT "perl $Bin/draw_tree_svg.pl $tree_nhx_file $Key_species; ",
				"perl  $Bin/display_muscle_ouput.pl  --seq_file $cds_muscle_file --annotation_file $annotation_file  --structure_file $structure_file --id_file $tree_id_file --exon_mark triangle --figure_width 1000 --left_margin 510 --right_margin 30  --top_margin 30 --bottom_margin 30 --join T   --coordinate --outdir $fam_dir/$sub_dir/$family_id; ",
