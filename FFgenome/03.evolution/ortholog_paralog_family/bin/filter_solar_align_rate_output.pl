...
print "query_id\tquery_len\tquery_align_rate\ttarget_id\ttarget_len\ttarget_align_rate\n";
...
while (<IN>) {
	...
	if ($query_align / $query_len >= $align_cutoff && $target_align / $target_len >= $align_cutoff) {
		##print $_;
		my $query_rate = $query_align / $query_len;
		my $target_rate = $target_align / $target_len;
		print "$query_id\t$query_len\t$query_rate\t$target_id\t$target_len\t$target_rate\n";
	}
}
close IN;
