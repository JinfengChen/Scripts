...
our %opt;
GetOptions (\%opt,"fasta:s","refer:s","project:s","help");


my $help=<<USAGE;
Find LRR domain in Pkinase, so the output should be RLK-LRR
perl $0 --fasta Pkinase.OS.fa --project rice --refer rice.RLK-LRR.fa
...
my $n=keys %final;
print "Blast Found $n\n";
open OUT, ">$opt{project}.Blast.found" or die "$!";
foreach(keys %final){
   print OUT "$_\n";
}
close OUT;
`perl ../getidseq.pl --list $opt{project}.Blast.found --fasta $opt{fasta} --output $opt{project}.Blast.found.fa`;
return \%final;
}
