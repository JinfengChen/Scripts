...
my $help=<<USAGE;
1. Check if gene are colinear among species.
2. Check if gene are moved in brd or rice.
...
open(IN,"$ARGV[0]");
while (<IN>) {
      ($bdid,$bdpos,$osid,$ospos,$sbid,$sbpos,$rem) = /^(\S+)\s+(\S+)\s+(Os\S+)\s+(\S+)\s+(Sb\S+)\s+(\S+)\s+(\S.*)/;
      if ($bdid) {
         ...
         &match_chr;
         $all_synt = 0;
         if (($os_sb_synt ==1) && ($os_bd_synt ==0) && ($bd_sb_synt ==0)) { ## movement in brd, colinear in os-sb
         #if ($os_bd_synt ==1 && $bd_sb_synt == 1) {  ## all colinear, most strictly
         #if ($os_bd_synt ==1) { ## only colinear between rice and brd
