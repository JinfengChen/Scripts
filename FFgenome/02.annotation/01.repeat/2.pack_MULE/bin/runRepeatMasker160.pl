...
my $outdir="./";
my $common="/home/jfchen/FFproject/tools/bin";
...
foreach (@subfiles){
    print "$_\n";
    unless (-f $lib){
       print OUT "$repeatmasker -species rice -x -nolow -no_is -norna -engine wublast $_ > $_.log 2> $_.log2\n";
    }else{
       print OUT "$repeatmasker -lib $lib -nolow -x -no_is -norna -engine crossmatch $_ > $_.log 2> $_.log2\n";
    }
}
close OUT;
