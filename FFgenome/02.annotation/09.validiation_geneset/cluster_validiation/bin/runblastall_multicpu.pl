...
my $outdir="./";
my $blastall="/usr/local/genome/bin/blastall";
...
foreach (@subfiles){
    print "$_\n";
    print OUT "$blastall -p $blast -i $_ -d $database -o $_.blast -e $evalue -F F > $_.log 2> $_.log2\n";
}
close OUT;

## run shell by qsub-sge.pl
`$multiprocess -cpu 8 $blastshell`;
`cat $outdir/$filename.cut/*.blast > $outdir/$filename.blast`;
`$blastparser $outdir/$filename.blast > $outdir/$filename.blast.table`;
