...
GetOptions (\%opt,"refgff:s","refpep:s","refgff3:s","reffasta:s","qrygff:s","qrypep:s","qrygff3:s","qryfasta:s","align:s","chr:s","help");


my $help=<<USAGE;

perl $0 -refgff Os.gff -refpep Os.pep -refgff3 Os.gff3.chr/chr01 -reffasta Os.fasta -qrygff Ob.gff -qrypep Ob.pep -qrygff3 Ob.gff3.chr/chr01 -qryfasta Ob.fasta -align ob_os.align -chr chr01
USAGE
...
$GFF2embl="$Bin/GFF2embl.pl";
$blast2seq="$Bin/runblast2seq.pl";
$toact="$Bin/run2act.pl";

`mkdir $Bin/../output/$opt{chr}` unless (-f "$Bin../output/$opt{chr}");
`mkdir $Bin/../output/$opt{chr}/tandem` unless (-f "$Bin/../output/$opt{chr}/tandem");
`mkdir $Bin/../output/$opt{chr}/linear` unless (-f "$Bin/../output/$opt{chr}/linear");
...
my $totalorth=$ortholog;
print "Total got ortholog: $totalorth\n";
...
system ("$blastall -p blastp -i qry.pep -d qry.pep -e 1e-10 -o qry_qry.blastm8 -m 8 > qry.log 2> qry.log2");
system ("$blastall -p blastp -i ref.pep -d ref.pep -e 1e-10 -o ref_ref.blastm8 -m 8 > ref.log 2> ref.log2");
my ($qrymost,$qryhomolog)=mosthitm8("qry_qry.blastm8");
my ($refmost,$refhomolog)=mosthitm8("ref_ref.blastm8");
#if ($qrymost >= 0){
if ($qrymost >= 3 or $refmost >= 3){
   print "Have tandem duplicate\n"; 
   $additionalpair=0;
   `mkdir $Bin/../output/$opt{chr}/tandem/$pairnum`;
   & createACT($qryhead,$refhead,"$Bin/../output/$opt{chr}/tandem/$pairnum");
   `mv *.blastm8 $Bin/../output/$opt{chr}/tandem/$pairnum`;
   `rm qry* ref*`;
}else{
   #`mkdir ../output/linear/$pairnum`;
   #& createACT($qryhead,$refhead,"../output/linear/$pairnum");
   ...
   foreach (keys %$orthpair){
      ...
      unless (exists $ortholog{$_}){
          print "$_\t$orthpair->{$_}\n";
          #$ortholog{$_}=$orthpair->{$_};
          $ortholog++;
      }
   }
   ####deal non-collinarity gene in qry
   & noncollinear($qrygene,$qrypep,"ref.pep",\%orthqry,$qryhomolog,$refhead,$pairnum,"FF.$opt{chr}");
   ####deal non-collinarity gene in ref
   & noncollinear($refgene,$refpep,"qry.pep",\%orthref,$refhomolog,$qryhead,$pairnum,"Rice.$opt{chr}");
   `rm qry* ref*`;   
}
...
sub noncollinear
{
my ($qrygene,$pep,$compep,$orthqry,$homolog,$refhead,$pairnum,$prefix)=@_;
if (@$qrygene > keys %$orthqry){
      foreach(@$qrygene){
         unless (exists $orthqry->{$_}){
            ...
            & getlistseq (\@tempgene, $pep, "temp.pep");
            system ("$blastall -p blastp -i temp.pep -d $compep -e 1e-10 -o temp_ref.blastm8 -m 8 > temp.blast.log 2> temp.blast.log2");
            ...
            if ($tempmost == 0){ ### if this gene have no homolog gene in ref region
                ...
                if ($temphit == 0){
                   ...
                   if (keys %$reftempgff == 0){
                       ...
                       foreach (keys %$reftempgff){
                           ...
                           unless ($overlap){
                              $newgff++;
                              & write2file($reftempgff->{$tempid}, "$prefix.add2.gff");
                           }
                       }
                       if ($newgff == 0){
                           #### this record gene in the same position of other gene, but miss anntated with wrong frame
                           & write2file("$testgene\t$pairnum\n", "$prefix.SingleMissAnnotation.id");
                       }else{
                           & write2file("$testgene\t$pairnum\n", "$prefix.SingleLost.id");
                       }
                   }
                }
            }else{
                ...
                if ($newgff == 0){
                     #### this record gene which is part of other gene, but miss annotated as a gene.
                     if ($homolog->{$testgene} > 1){
                         & write2file("$testgene\t$pairnum\n", "$prefix.duplcated.id");
                     }else{
                         & write2file("$testgene\t$pairnum\n", "$prefix.partofgene.id");
                     }
                }else{
                     & write2file("$testgene\t$pairnum\n", "$prefix.FamilyLost.id");
                }
            }
            #`mkdir ../output/linear/$pairnum/$testgene`;
            #`mv temp* ../output/linear/$pairnum/$testgene/`;
            `rm -R $refhead*`;
         }
      }
}
...
getsubfasta($opt{reffasta},$refhead,"+");
`perl $GFF2embl -gff $qryhead.gff -embl $qryhead.embl -fasta $qryhead.fasta`;
`perl $GFF2embl -gff $refhead.gff -embl $refhead.embl -fasta $refhead.fasta`;
`perl $blast2seq`;
`perl $toact`;
