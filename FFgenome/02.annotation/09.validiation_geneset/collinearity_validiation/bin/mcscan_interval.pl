...
`mkdir ../output/tandem` unless (-f "../output/tandem");
`mkdir ../output/linear` unless (-f "../output/linear");
...
if ($qrymost >= 3 or $refmost >= 3){
   ...
   $additionalpair=0;
   `mkdir ../output/tandem/$pairnum`;
   & createACT($qryhead,$refhead,"../output/tandem/$pairnum");
   `mv *.blastm8 ../output/tandem/$pairnum`;
   `rm qry* ref*`;
}else{
   ...
   & noncollinear($qrygene,$qrypep,"ref.pep",\%orthqry,$qryhomolog,$refhead,$pairnum,"FF");
   ####deal non-collinarity gene in ref
   & noncollinear($refgene,$refpep,"qry.pep",\%orthref,$refhomolog,$qryhead,$pairnum,"Rice");
   `rm qry* ref*`;   
}
