...
{
my ($chr)=@_;
`find $opt{bed} | grep "CACTA.wave.fill.bed" | grep $chr > $opt{project}.$chr.CACTA.50000.list`;
`find $opt{bed} | grep "MUDR.wave.fill.bed" | grep $chr > $opt{project}.$chr.MUDR.50000.list`;
`find $opt{bed} | grep "MITE.wave.fill.bed" | grep $chr > $opt{project}.$chr.MITE.50000.list`;
`find $opt{bed} | grep "COPIA.wave.fill.bed" | grep $chr > $opt{project}.$chr.COPIA.50000.list`;
`find $opt{bed} | grep "GYPSY.wave.fill.bed" | grep $chr > $opt{project}.$chr.GYPSY.50000.list`;
`find $opt{bed} | grep "DNA.wave.fill.bed" | grep $chr > $opt{project}.$chr.DNA.50000.list`;
`find $opt{bed} | grep "RT.wave.fill.bed" | grep $chr > $opt{project}.$chr.RT.50000.list`;
`find $opt{bed} | grep "GENE.wave.fill.bed" | grep $chr > $opt{project}.$chr.GENE.50000.list`;
}

sub HMMSeg
{
my ($chr)=@_;
`java -jar HMMSeg.jar --input-bed --smooth-only 200000 --log hmm.log $opt{project}.$chr.GENE.50000.list > hmm.log1 2> hmm.log2`;
#`java -jar HMMSeg.jar --input-bed --smooth-only 200000 --log hmm.log $opt{project}.$chr.CACTA.50000.list $opt{project}.$chr.MUDR.50000.list $opt{project}.$chr.MITE.50000.list $opt{project}.$chr.COPIA.50000.list $opt{project}.$chr.GYPSY.50000.list $opt{project}.$chr.DNA.50000.list $opt{project}.$chr.RT.50000.list $opt{project}.$chr.GENE.50000.list > hmm.log1 2> hmm.log2`;
}

sub wave2bar
{
my @wave=glob("$opt{bed}/*.smooth.bed");
foreach my $file (@wave){
     my $head;
     if ($file=~/(.*)\.smooth.bed/){
        $head=$1;
     }
     open BAR, ">$head.smooth.bar.txt" or die "$!";
     open WAVE, "$file" or die "$!";
     while(<WAVE>){
         chomp $_;
         my @unit=split("\t",$_);
         print BAR "$unit[0]\t$unit[1]\t$unit[4]\n";
     }
     close WAVE;
     close BAR;
}
