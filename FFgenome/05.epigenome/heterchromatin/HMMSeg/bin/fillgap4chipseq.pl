...
GetOptions(\%opt,"window:s","smooth:s","chrlen:s","bed:s","project:s","help");
...
{
my ($chr)=@_;
`find $opt{bed} | grep "H3K9me2.wave.fill.bed" | grep $chr > $opt{project}.$chr.H3K9me2.50000.list`;
}
...
`java -jar HMMSeg.jar --num-states 2 --input-bed --smooth $opt{smooth} --num-starts 10 --log hmm.log $opt{project}.$chr.H3K9me2.50000.list > hmmseg.log1 2> hmmseg.log2`;
}
