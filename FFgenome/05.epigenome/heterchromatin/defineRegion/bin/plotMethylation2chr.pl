...
GetOptions (\%opt,"chrlen:s","bar:s","type:s","project:s","help");


my $help=<<USAGE;
Plot features along chromosomes.
Run: perl $0 --chrlen ../input/IRGSP.chrlen --bar ../input/rice_methylation_bar --type CG --project rice
--chrlen file contains chr length for each chromosome
--bar directory contains bar file for all chromosome
--type draw this type of methylation only, all means draw all in one figure
--project title
USAGE
...
if ($opt{type} eq "all"){
   ...
   if (-f $chg){
      ...
      $svg=drawline($svg,$chgvalue,"yellow","2","CHG",$ystartvalue,$ylength,$chrlen);
   }
   my $chh="$opt{bar}/$name.CHH.bar.txt";
   if (-f $chh){
      my $chhvalue=value($chh);
      $svg=drawline($svg,$chhvalue,"blue","3","CHH",$ystartvalue,$ylength,$chrlen);
   }
}
print "Writing file\n";
