...
GetOptions (\%opt,"chrlen:s","bar:s","type:s","epi","project:s","help");


my $help=<<USAGE;
Plot features along chromosomes.
Run: perl plotFeature2chr.pl --chrlen ../input/IRGSP.chrlen --bar ../input/rice_feature_bar --type all --epi  --project rice > log 2> log2 &
...
--epi if to plot epigenetic signals
--project title
USAGE
...
my $ystartvalue=0; ##y axis start
my $ylength=100; ##y axis length
my $ycut=$ylength/10;
...
#drawpoint(\@gc);
print "Drawing line\n";
$svg=drawline($svg,\%pos2value,$color,$legandn,$legandnote,$ystartvalue,$ylength,$chrlen);
if ($opt{type} eq "all"){
...
if ($opt{epi}){
   my $cg="$opt{bar}/$name.CG.bar.txt";
   if (-f $cg){
      my $cgvalue=value($cg);
      $svg=drawline($svg,$cgvalue,"green","3","CG",$ystartvalue,$ylength,$chrlen);
   }
   my $chg="$opt{bar}/$name.CHG.bar.txt";
   if (-f $chg){
      my $chgvalue=value($chg);
      $svg=drawline($svg,$chgvalue,"olive","4","CHG",$ystartvalue,$ylength,$chrlen);
   }
   my $chh="$opt{bar}/$name.CHH.bar.txt";
   if (-f $chh){
      my $chhvalue=value($chh);
      $svg=drawline($svg,$chhvalue,"purple","5","CHH",$ystartvalue,$ylength,$chrlen);
   }
}
