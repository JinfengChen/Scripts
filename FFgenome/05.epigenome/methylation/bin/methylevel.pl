...
foreach(@genebed){
   ...
   `/home/jfchen/FFproject/tools/BEDTools/bin/windowBed -a $methbed -b $tempbed -w 2000 > chr.win`;
...
my (@body,@bodyC,@bodymC);
my @part=(0,0,0,0,0,0);
my $length=$end-$start+1;
my $interval=$length*0.05;
open WIN, "$win" or die "$!"; 
while(<WIN>){
    chomp $_;
    next if ($_ eq "");
    ...
    if ($unit[1] < $start){
        ...
        my $index=int (($unit[1]-$start)/$interval);
        $bodyC[$index]++;
        $part[2]++;
        if ($unit[3] > $unit[4]){
           $bodymC[$index]++;
