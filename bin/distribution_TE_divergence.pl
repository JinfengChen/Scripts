...
die "Usage:perl $0 <.out> <Genomo_size>" if(@ARGV!=2 || $help);
$X_step ||= 1;
$X_end ||= 40;
$Y_step ||= 0.5;
$Y_end ||= 3;
...
sub read_out{
	...
	while(<IN>)
	{
			...
			if($_[10]=~/LINE/)
			{
					$p->[$div]{'LINE'}+=abs($_[5]-$_[6]);
			}
			elsif($_[10]=~/SINE/)
			{
					$p->[$div]{'SINE'}+=abs($_[5]-$_[6]);
			}
			elsif($_[10]=~/LTR/)
			{
					$p->[$div]{'LTR'}+=abs($_[5]-$_[6]);
			}
			elsif($_[10]=~/DNA/)
			{
					$p->[$div]{'DNA'}+=abs($_[5]-$_[6]);
			}
			else
			{
					next;
...
################ Tag ###########################################
	...
	$svg->rect('x',200+$width-$r_width,'y',108,'width',$h_width,'height',$h_width,'fill','red','stroke-width',2,'stroke','black');
	$svg->text('x',200+$width-$r_width+5+$h_width,'y',105+$h_width,'-cdata',"DNA",'font-family',$font_family,'font-size',$font_size);
	$svg->rect('x',200+$width-$r_width,'y',108+$h_width+5,'width',$h_width,'height',$h_width,'fill','orange','stroke-width',2,'stroke','black');
	$svg->text('x',200+$width-$r_width+5+$h_width,'y',105+$h_width*2+5,'-cdata',"LINE",'font-family',$font_family,'font-size',$font_size);
	$svg->rect('x',200+$width-$r_width,'y',108+$h_width*2+10,'width',$h_width,'height',$h_width,'fill','yellow','stroke-width',2,'stroke','black');
	$svg->text('x',200+$width-$r_width+5+$h_width,'y',105+$h_width*3+10,'-cdata',"LTR",'font-family',$font_family,'font-size',$font_size);
	$svg->rect('x',200+$width-$r_width,'y',108+$h_width*3+15,'width',$h_width,'height',$h_width,'fill','green','stroke-width',2,'stroke','black');
	...
	foreach my $i(0..$X_end-1)
	{
		...
		$svg->rect('x',$x,'y',$y,'width',$w,'height',$per_SINE*$y_resolution,'fill','green','stroke-width',2);
		$y-=$per_LTR*$y_resolution;
		$svg->rect('x',$x,'y',$y,'width',$w,'height',$per_LTR*$y_resolution,'fill','yellow','stroke-width',2);
		$y-=$per_LINE*$y_resolution;
		$svg->rect('x',$x,'y',$y,'width',$w,'height',$per_LINE*$y_resolution,'fill','orange','stroke-width',2);
		$y-=$per_DNA*$y_resolution;
		$svg->rect('x',$x,'y',$y,'width',$w,'height',$per_DNA*$y_resolution,'fill','orange','stroke-width',2);
